...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.substring(0, position) + str.substring(position).replace(value, '')
  );
}
...
function countVowels(str) {
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (/[aeiouyAEIOUY]/.test(str[i])) count += 1;
  }
  return count;
}
...
function isPalindrome(str) {
  const res = str.replaceAll(/[\s\W,.:?!]/g, '').toLowerCase();
  for (let i = 0, j = res.length - 1; i < j; i += 1, j -= 1) {
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < splitted.length; i += 1) {
    if (splitted[i].length > longest.length) longest = splitted[i];
  }
  return longest;
}
...
function invertCase(str) {
  return [].map
    .call(str, function check(element) {
...
function encodeToRot13(str) {
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    let code = /\w/.test(str[i]) ? str.charCodeAt(i) + 13 : str.charCodeAt(i);
    if ((code >= 91 && code <= 103) || code > 122) {
      code -= 26;
    }
